...
const projects = [
  ...
  { name: 'OpenGL Annimation', type: 'cpp', makePath: 'OpenGL', executable: './sample', port: 4000 },
...
// Serve the Rock-Paper-Scissors HTML file directly
app.get('/rock-paper-scissors', (req, res) => {
  res.sendFile(path.join(__dirname, 'rock_paper_scissors.html'));
});


// Route to handle OpenGL project execution
app.get('/run-final', (req, res) => {
  const project = projects.find(p => p.name === 'OpenGL Annimation');
